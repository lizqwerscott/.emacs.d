...
7. dtach for detached
8. tail for detached
9. [[https://direnv.net/][direnv]] for env
*** Need build tree-sitter library
Use ~+treesit-install-required-language-grammars~ install required language grammars.
...
Use ~customize-save-variable~ to set custom variables, and use ~customize-theme~ to select themes.
You can use ~customize-group~ to view the options that can be set by users.
