...
(defcustom fussy-orderless-affix-dispatch-alist '((?= . fussy-orderless-chinese-regexp-score)
  ...
  :type `(alist
          ...
          :value-type (choice
                       ...
                       (const :tag "Literal" ,#'fussy-orderless-literal-score)
...
(defun fussy-orderless--split-string-with-prefixs (query prefixs)
  ...
  (let ((querys (split-string query))
        ...
        (index 0))
    (dolist (item querys)
      (if-let* ((_ (> (length item) 1))
                (prefix (cl-find item prefixs :test (lambda (item prefix) (string-prefix-p prefix item)))))
...
(defun fussy-orderless-literal-score (string query)
  "Use QUERY and STRING calc score."
  (let* ((case-fold-search t)
         (query-len (length query))
         (string-len (length string)))
    (when (and (> query-len 0) (>= string-len query-len))
      (if-let* ((pos (string-match query string)))
          (list (round (* 100 (/ (float (+ (- string-len pos) 1))
                                 string-len)))
                pos
                (+ pos query-len))
        (list 0)))))
...
(defun fussy-orderless-normal-score (str normal-items items-len normal-score-fn &optional args)
